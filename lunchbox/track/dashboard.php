								...
								<div class="card-body">
									<table class="table table-responsive-md table-striped mb-0">
                                        ...
                                        <?php
										  ...
										  if($eid==0)
										  {
        								   $subs_res=mysqli_query($conn, "SELECT * from subscriptions order by serial;"); 		
                                          }
										  else
										  {
        								   $subs_res=mysqli_query($conn, "SELECT * from subscriptions where delivery_partner=$eid order by serial;"); 		
                                          }
										  
										  $sno=1;
          								  $first_unpicked = false; // place this before the loop starts
...
while($row = mysqli_fetch_array($subs_res))
{
    ...
    // ✅ Scroll target for first NOT PICKED UP row
    if ($status == 0 && !$first_unpicked) {
        echo "<tr id='scrollTarget'>";
        $first_unpicked = true;
    } else {
        echo "<tr>";
    }
    ...
    if ($status == 0) {
        echo "<td id='gerr$pid' style='width: 30%'><a href='#' onclick='change_status($pid,$school);'><button type='button' class='mb-1 mt-1 mr-1 btn btn-xs btn-success'>PICK</button></a><div style='font-size:12px;'>Call Parent @<br><a href='tel:$pid'><i class='fa fa-phone'></i> ".$pid."</a></div></td>";
    } else if ($status == 1) {
        $pick_time = $srow['pickup_time'];
        $pickup_time = date('g:i A', strtotime($pick_time));
        echo "<td id='gerr$pid'><a href='#' onclick='change_status($pid,$school);'><button type='button' class='mb-1 mt-1 mr-1 btn btn-xs btn-warning'>DROP</button></a>";
        echo "<div style='font-size:10px;'>Picked up at <i class='fa fa-clock' style='font-size:10px;'></i> $pickup_time</div><div style='font-size:12px;'>Call Parent @<br><a href='tel:$pid'><i class='fa fa-phone'></i> ".$pid."</a></div></td>";
    } else {
        $pick_time = $srow['pickup_time'];
        $pickup_time = date('g:i A', strtotime($pick_time));
        ...
        echo "<div style='font-size:10px;line-height:10px;'>Picked up at <i class='fa fa-clock' style='font-size:10px;'></i> $pickup_time<br>";
